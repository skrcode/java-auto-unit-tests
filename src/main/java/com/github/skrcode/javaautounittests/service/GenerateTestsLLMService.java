    ...
    public static PromptResponseOutput generatePlan(
        ...
        while (true) {
            try {
                ...
                indicator.checkCanceled();
                HttpRequest createJobReq = HttpRequest.newBuilder()
                        .uri(URI.create("https://otxfylhjrlaesjagfhfi.supabase.co/functions/v1/invoke-junit-llm-fetch-plan"))
                        .timeout(Duration.ofSeconds(30))
                        .header("Accept", "application/json")
                        .header("Content-Type", "application/json")
                        .header(headerName, headerValue == null ? "" : headerValue)
                ...
                PromptResponseOutput planOutput = MAPPER.treeToValue(jsonNode, PromptResponseOutput.class);;
                if(planOutput == null || planOutput.getMessage() == null || CollectionUtils.isEmpty(planOutput.getMessage().getContentAsList()) || planOutput.getMessage().getContentAsList().get(0) == null)
